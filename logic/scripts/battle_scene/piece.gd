...
func set_interference(interference_t: String, hp := 1, non_support_pokemon_id: int = -1):
	...
	match interference_t:
		"barrier":
			stop_idle_animation()
			if pokemon_sprite != null:
				pokemon_sprite.modulate = Color(0.7, 0.9, 1.0, 1.0)
			update_interference_visual()
		...
		"non_support_pokemon":
			# 🔥 CAMBIAR COMPLETAMENTE EL POKÉMON (con forma si aplica)
			if non_support_pokemon_id != -1:
				is_non_support_pokemon = true
				
				var id_string = str(non_support_pokemon_id)
				var parsed = {}
				if "_" in id_string:
					var parts = id_string.split("_")
					parsed = {"pokemon_id": int(parts[0]), "form_id": int(parts[1])}
				else:
					parsed = {"pokemon_id": int(id_string), "form_id": 0}

				pokemon_id = parsed["pokemon_id"]
				pokemon_form = parsed["form_id"]
				pokemon_level = 1
...
func update_interference_visual():
	...
	match interference_type:
		...
		"barrier":
			texture_path = "res://assets/sprites/grid/disruptions/barrier.png"
