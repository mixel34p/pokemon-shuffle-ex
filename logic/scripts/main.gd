	...
	while safety_counter < MAX_COMBOS:
		...
		for piece in matches:
			...
			if piece.interference_type == "barrier":
				piece.interference_type = ""
				piece.interference_hp = 1
				if piece.interference_overlay != null:
					piece.interference_overlay.queue_free()
					piece.interference_overlay = null
	...
	for y in range(GRID_HEIGHT):
		for x in range(GRID_WIDTH):
			...
			for i in range(x + 1, GRID_WIDTH):
				if grid[y][i] != null and grid[y][i].is_matchable() and grid[y][i].type == piece_type:
					h_count += 1
			...
			for i in range(y + 1, GRID_HEIGHT):
				if grid[i][x] != null and grid[i][x].is_matchable() and grid[i][x].type == piece_type:
					v_count += 1
...
func apply_disruption_to_piece(piece: PokemonPiece, disruption_type: String, hp: int, pokemon_id: int):
	...
	match disruption_type:
		...
		"non_support_pokemon":
			...
			# Si no se especificó un pokemon_id, elegir uno aleatorio que no esté en el equipo
			if pokemon_id == -1:
				var available_pokemon = []
				for pokemon_key in pokemon_database.keys():
					var pokemon_id_int = int(pokemon_key)
					if not is_pokemon_in_team(pokemon_id_int):
						available_pokemon.append(pokemon_id_int)
				
				if available_pokemon.size() > 0:
					pokemon_id = available_pokemon[randi() % available_pokemon.size()]
				else:
					pokemon_id = 19  # Fallback
			...
			new_piece.setup(x, y, 4, TILE_SIZE, pokemon_id, 1, 0)
				...
				if grid[new_row][new_col] != null:
					...
					if adjacent_piece.interference_type in ["rock", "block"]:
						if adjacent_piece.weaken_interference():
							adjacent_piece.animate_match_pop()
							await get_tree().create_timer(0.2).timeout
							grid[new_row][new_col] = null
							adjacent_piece.queue_free()
